...
def ensure_huma_schema(db_path: str) -> None:
    """
    Crée les tables pour la base de données humanitaire :
    - calls : table des appels
    - grh_hours : table des heures de travail GRH
    - objectifs : table des objectifs par agent
    """
    parent = os.path.dirname(db_path)
    if parent and not os.path.exists(parent):
        os.makedirs(parent, exist_ok=True)
    ...
    try:
        ...
        conn.commit()
        print(f"[MIGRATION] ✅ Schéma humanitaire vérifié dans {db_path}")
    finally:
        conn.close()
